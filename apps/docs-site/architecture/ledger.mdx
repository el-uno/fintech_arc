---
title: "The Arc Ledger: Double-Entry Accounting With No Tolerance"
sidebarTitle: "Ledger"
...
icon: "scale-balanced"
keywords: ["double-entry ledger", "chart of accounts", "ledger invariants", "derived balances", "append-only entries", "Postgres constraint trigger"]
---
...
There is also a **known race**: between the balance read and the append, a concurrent journal could spend the same funds. In-memory and single-threaded this cannot happen. `PrismaLedgerStore` must close it with `SELECT … FOR UPDATE` on the touched accounts or a serializable transaction, because the database's balance trigger catches unbalanced journals but does **not** catch a race that overdraws an account.
</div>
...
<div className="arc-gap">
- **The `PrismaLedgerStore` adapter now exists**, so the engine is no longer in-memory only. The fast test suite still runs against the in-memory store by choice.
