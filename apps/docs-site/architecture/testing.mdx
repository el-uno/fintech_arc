...
icon: "flask"
keywords: ["property-based testing", "mutation testing", "chaos engineering", "ledger invariants", "test strategy"]
---
...
- The property suite runs against the in-memory store for speed. `PrismaLedgerStore` exists and the database constraints are verified by raw SQL against a live database, but the property suite and the real constraints are not yet exercised together in a single run.
- There is a **race** between the overdraft balance read and the append. Single-threaded and in-memory it cannot occur; `PrismaLedgerStore` must close it with `SELECT … FOR UPDATE` or a serializable transaction. The database's balance trigger catches unbalanced journals but **not** a race that overdraws an account.
