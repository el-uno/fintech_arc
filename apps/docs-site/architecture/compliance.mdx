...
icon: "shield"
keywords: ["KYC KYB", "sanctions screening", "AML rules", "Jaro-Winkler name matching", "four-eyes approval", "transaction monitoring"]
---

<span className="arc-eyebrow">Architecture · the core</span>

<Info>
**Phase 5 is complete.** The KYC tiering, sanctions screener, AML rule engine and review queue are implemented and tested, and the compliance gate is wired into the saga as a hard pre-condition. The permissive `AlwaysApprove` implementation still ships as the default used by the movement test harness, so a composition root that does not supply a real `CompliancePort` gets an open gate.
</Info>
...
`AlwaysApprove` remains in the codebase as the movement test harness's stand-in, so a composition root that forgets to supply a real `CompliancePort` silently gets an open gate. That is a wiring hazard rather than a design position: the gate itself is a hard pre-condition in the saga. Compliance-blocked paths are also not yet in the chaos suite, so the unwind on a mid-saga rejection is reasoned about rather than asserted.
</div>
