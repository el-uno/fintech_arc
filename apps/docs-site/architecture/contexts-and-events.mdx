...
icon: "inbox"
keywords: ["transactional outbox", "at-least-once delivery", "event-driven architecture", "dependency-cruiser", "idempotent handlers"]
---
...
`CompliancePort` works the same way. Its permissive default, `AlwaysApprove`, exists so the saga was testable before the risk context did, and it is explicitly *not* a decision that compliance is optional: the gate is wired as a hard pre-condition either way. The risk context now supplies the real implementation, and `AlwaysApprove` remains only as the movement test harness's stand-in.
</Note>
